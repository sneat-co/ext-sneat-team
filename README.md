...
`ext-sneat-team` depends **only on foundational/core code — never on another extension.** Because it
has no edge back to any sibling, `sibling → ext-sneat-team` can never form a dependency cycle. An
...
via standard `go get github.com/sneat-co/ext-sneat-team/backend` once published.
